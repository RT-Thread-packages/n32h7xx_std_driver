...
#ifndef __N32H7XX_ETH_H__
#define __N32H7XX_ETH_H__
...
typedef enum
{
    ...
    ETH_TX_PACKETS_FEATURES_TTSE         = ((uint32_t)0x00000080U)  /* Transmit Timestamp Enable */
} EthTxPackAttrType;
...
typedef enum
{
    ETH_INNERVLANTAGRXSTRIPPING_NONE    = ((uint32_t)0x00000000U),
    ETH_INNERVLANTAGRXSTRIPPING_IFPASS  = ((uint32_t)0x10000000U),
    ETH_INNERVLANTAGRXSTRIPPING_IFFAILS = ((uint32_t)0x20000000U),
    ETH_INNERVLANTAGRXSTRIPPING_ALWAYS  = ((uint32_t)0x30000000U)
} EthRxInVlanStripType;
...
typedef enum
{
    ETH_TXVLAN_CVLAN = ((uint32_t)0x00000000U),
    ETH_TXVLAN_SVLAN = ((uint32_t)0x00080000U)
} EthTxVlanType;

typedef enum
{
    ETH_TXVLANTAGCONTROL_NONE    = ((uint32_t)0x00040000U),
    ETH_TXVLANTAGCONTROL_DELETE  = ((uint32_t)0x00050000U),
    ETH_TXVLANTAGCONTROL_INSERT  = ((uint32_t)0x00060000U),
    ETH_TXVLANTAGCONTROL_REPLACE = ((uint32_t)0x00070000U)
} EthTxVlanCtrlType;

typedef enum
{
    ETH_LPITXCLKSTOP_DISABLE = ((uint32_t)0x00000000U),
    ETH_LPITXCLKSTOP_ENABLE  = ((uint32_t)0x00200000U)
} EthLPITxClkType;

typedef enum
{
    ETH_LPITXAUTOMATE_DISABLE = ((uint32_t)0x00000000U),
    ETH_LPITXAUTOMATE_ENABLE  = ((uint32_t)0x00080000U)
} EthLPITxAutoType;

typedef enum
{
    ETH_L3_PROTOCOL_IPV4 = ((uint32_t)0x00000000U),
    ETH_L3_PROTOCOL_IPV6 = ((uint32_t)0x00000001U)
} EthL3ProtocolType;

typedef enum
{
    ETH_L3_SRC_ADDR_MATCH_DISABLE = ((uint32_t)0x00000000U),
    ETH_L3_SRC_ADDR_PERFECT_MATCH = ((uint32_t)0x00000004U),
    ETH_L3_SRC_ADDR_INVERSE_MATCH = ((uint32_t)0x0000000CU)
} EthL3SAMatchType;

typedef enum
{
    ETH_L3_DEST_ADDR_MATCH_DISABLE = ((uint32_t)0x00000000U),
    ETH_L3_DEST_ADDR_PERFECT_MATCH = ((uint32_t)0x00000010U),
    ETH_L3_DEST_ADDR_INVERSE_MATCH = ((uint32_t)0x00000030U)
} EthL3DAMatchType;

typedef enum
{
    ETH_L4_PROTOCOL_TCP = ((uint32_t)0x00000000U),
    ETH_L4_PROTOCOL_UDP = ((uint32_t)0x00010000U)
} EthL4ProtocolType;

typedef enum
{
    ETH_L4_SRC_PORT_MATCH_DISABLE = ((uint32_t)0x00000000U),
    ETH_L4_SRC_PORT_PERFECT_MATCH = ((uint32_t)0x00040000U),
    ETH_L4_SRC_PORT_INVERSE_MATCH = ((uint32_t)0x000C0000U)
} EthL4SPMatchType;

typedef enum
{
    ETH_L4_DEST_PORT_MATCH_DISABLE = ((uint32_t)0x00000000U),
    ETH_L4_DEST_PORT_PERFECT_MATCH = ((uint32_t)0x00100000U),
    ETH_L4_DEST_PORT_INVERSE_MATCH = ((uint32_t)0x00300000U)
} EthL4DPMatchType;
...
/** ETH Rx VLAN tag configuration structure definition **/
typedef struct
{
    uint32_t             InnerVLANTagInStatus;   /* Enables or disables Inner VLAN Tag in Rx Status */

    EthRxInVlanStripType StripInnerVLANTag;      /* Sets the Inner VLAN Tag Stripping on Receive */

    uint32_t             InnerVLANTag;           /* Enables or disables Inner VLAN Tag */

    uint32_t             DoubleVLANProcessing;   /* Enable or Disable double VLAN processing */
...
/** ETH Tx VLAN tag configuration structure definition **/
typedef struct
{
    uint32_t          VLANTagInput; /* Enable or Disable VLAN tag source from DMA tx descriptors */

    EthTxVlanType     VLANType;     /* Sets the insertion of VLAN type (C-VLAN or S-VLAN) */

    EthTxVlanCtrlType VLANTagCtrl;  /* Sets the VLAN tag control in tx packets */
} ETH_TxVLANTagType;
...
typedef struct
{
    ...
    uint32_t        SAInsertCtrl;   /* Specifies the source address insertion control.
                                       This parameter must be one of the following values:
    ...
    uint32_t        CRCPadCtrl;     /* Specifies the CRC and Pad insertion and replacement control.
                                       ...
                                       - ETH_CRC_PAD_INSERT
                                       - ETH_CRC_INSERT
                                       - ETH_CRC_INSERT_DISABIE
                                       - ETH_CRC_REPLACEMENT
                                    */

    uint32_t        ChecksumCtrl;   /* Specifies the checksum insertion control.
                                       This parameter must be one of the following values:
                                       - ETH_CHECKSUM_INSERT_DISABIE
                                       - ETH_CHECKSUM_INSERT_ONLY_IPHDR
                                       - ETH_CHECKSUM_INSERT_IPHDR_PAYLOAD
                                       - ETH_CHECKSUM_INSERT_IPHDR_PAYLOAD_PHDR_CALC
                                    */
    ...
    uint32_t        InnerVlanTag;   /* Sets Inner VLAN Tag only when VLAN is enabled */

    uint32_t        InnerVlanCtrl;  /* Specifies Inner VLAN Tag insertion control only when Inner VLAN is enabled.
                                       This parameter must be one of the following values:
                                       - ETH_TX_INNER_VLAN_DISABLE
                                       - ETH_TX_INNER_VLAN_REMOVE
                                       - ETH_TX_INNER_VLAN_INSERT
                                       - ETH_TX_INNER_VLAN_REPLACE
                                    */
} ETH_TxPacketType;
...
typedef struct
{
    ...
    uint32_t PayloadType;     /* Payload type.
                                 This parameter can be one of the following values:
                                 - ETH_IP_PAYLOAD_TYPE_UNKNOWN
                                 - ETH_IP_PAYLOAD_TYPE_UDP
                                 - ETH_IP_PAYLOAD_TYPE_TCP
                                 - ETH_IP_PAYLOAD_TYPE_ICMPN
                              */
    ...
    uint32_t L3FilterStatus;  /* L3 filter status.
                                 ...
                                 - ETH_L3_FILTER2_MATCH
                                 - ETH_L3_FILTER3_MATCH
                                 - ETH_L3_FILTER4_MATCH
                                 - ETH_L3_FILTER5_MATCH
                                 - ETH_L3_FILTER6_MATCH
                                 - ETH_L3_FILTER7_MATCH
                              */
    ...
    uint32_t L4FilterStatus;  /* L4 filter status.
                                 ...
                                 - ETH_L4_FILTER2_MATCH
                                 - ETH_L4_FILTER3_MATCH
                                 - ETH_L4_FILTER4_MATCH
                                 - ETH_L4_FILTER5_MATCH
                                 - ETH_L4_FILTER6_MATCH
                                 - ETH_L4_FILTER7_MATCH
                              */
...
/** ETH L3 filter structure definition **/
typedef struct
{
    EthL3ProtocolType Protocol;           /* Sets the L3 filter protocol to IPv4 or IPv6 */

    EthL3SAMatchType SrcAddrFilterMatch;  /* Sets the L3 filter source address match */

    EthL3DAMatchType DestAddrFilterMatch; /* Sets the L3 filter destination address match */
...
/** ETH L4 filter structure definition **/
typedef struct
{
    EthL4ProtocolType Protocol;           /* Sets the L4 filter protocol to TCP or UDP */

    EthL4SPMatchType SrcPortFilterMatch;  /* Sets the L4 filter source port match */

    EthL4DPMatchType DestPortFilterMatch; /* Sets the L4 filter destination port match */

    uint32_t SrcPort;                     /* Sets the L4 filter source port
                                             This parameter must be a value from 0x0 to 0xFFFF */

    uint32_t DestPort;                    /* Sets the L4 filter destination port
                                             This parameter must be a value from 0x0 to 0xFFFF */
} ETH_L4FilterType;
...
#define ETH_LINK_10FULL        ((uint16_t)0x0001U)
#define ETH_LINK_10HALF        ((uint16_t)0x0002U)
#define ETH_LINK_100FULL       ((uint16_t)0x0004U)
#define ETH_LINK_100HALF       ((uint16_t)0x0008U)
#define ETH_LINK_1000FULL      ((uint16_t)0x0010U)
#define ETH_LINK_1000HALF      ((uint16_t)0x0020U)
...
#define ETH_MACADDR_MASK_NONE  ((uint32_t)(ETH_MACADDR1H_MBC_0 \
                                         & ETH_MACADDR1H_MBC_1 \
                                         & ETH_MACADDR1H_MBC_2 \
                                         & ETH_MACADDR1H_MBC_3 \
                                         & ETH_MACADDR1H_MBC_4 \
...
#define ETH_DMATXND0RF_BUF1AP ((uint32_t)0xFFFFFFFFU) /* Buffer 1 Address Pointer */
#define ETH_DMATXND0RF_TSOHAP ((uint32_t)0xFFFFFFFFU) /* TSO Header Address Pointer */

/** Bit definition for ETH_DMATXND1RF register (TDES1) **/
#define ETH_DMATXND1RF_BUF2AP ((uint32_t)0xFFFFFFFFU) /* Buffer 2 Address Pointer */

/** Bit definition for ETH_DMATXND2RF register (TDES2) **/
#define ETH_DMATXND2RF_B1L      ((uint32_t)0x00003FFFU) /* Buffer 1 Length */
#define ETH_DMATXND2RF_HL       ((uint32_t)0x00003FFFU) /* Header length when TCP segmentation is enabled */
#define ETH_DMATXND2RF_VTIR     ((uint32_t)0x0000C000U) /* VTIR[1:0]: VLAN Tag Insertion or Replacement */
#define ETH_DMATXND2RF_VTIR_0   ((uint32_t)0x00004000U) /* VTIR bit 0 */
#define ETH_DMATXND2RF_VTIR_1   ((uint32_t)0x00008000U) /* VTIR bit 1 */
#define ETH_DMATXND2RF_B2L      ((uint32_t)0x3FFF0000U) /* Buffer 2 Length */
#define ETH_DMATXND2RF_TTSE     ((uint32_t)0x40000000U) /* Transmit Timestamp Enable */
#define ETH_DMATXND2RF_IOC      ((uint32_t)0x80000000U) /* Interrupt on Completion */

/** Bit definition for ETH_DMATXND3RF register (TDES3) **/
#define ETH_DMATXND3RF_FL     ((uint32_t)0x00007FFFU) /* Frame Length */
#define ETH_DMATXND3RF_TPL    ((uint32_t)0x0003FFFFU) /* TCP Payload Length */
#define ETH_DMATXND3RF_CIC    ((uint32_t)0x00030000U) /* CIC[1:0]: Checksum Insertion Control */
#define ETH_DMATXND3RF_CIC_0  ((uint32_t)0x00010000U) /* CIC bit 0 */
#define ETH_DMATXND3RF_CIC_1  ((uint32_t)0x00020000U) /* CIC bit 1 */
#define ETH_DMATXND3RF_TSE    ((uint32_t)0x00040000U) /* TCP Segmentation Enable */
#define ETH_DMATXND3RF_THL    ((uint32_t)0x00780000U) /* TCP/UDP Header Length */
#define ETH_DMATXND3RF_SAIC   ((uint32_t)0x03800000U) /* SAIC[2:0]: SA Insertion Control */
#define ETH_DMATXND3RF_SAIC_0 ((uint32_t)0x00800000U) /* SAIC bit 0 */
#define ETH_DMATXND3RF_SAIC_1 ((uint32_t)0x01000000U) /* SAIC bit 1 */
#define ETH_DMATXND3RF_SAIC_2 ((uint32_t)0x02000000U) /* SAIC bit 2 */
#define ETH_DMATXND3RF_CPC    ((uint32_t)0x0C000000U) /* CPC[1:0]: CRC Pad Control */
#define ETH_DMATXND3RF_CPC_0  ((uint32_t)0x04000000U) /* CPC bit 0 */
#define ETH_DMATXND3RF_CPC_1  ((uint32_t)0x08000000U) /* CPC bit 1 */
#define ETH_DMATXND3RF_LD     ((uint32_t)0x10000000U) /* Last Descriptor */
#define ETH_DMATXND3RF_FD     ((uint32_t)0x20000000U) /* First Descriptor */
#define ETH_DMATXND3RF_CTXT   ((uint32_t)0x40000000U) /* Context Type */
#define ETH_DMATXND3RF_OWN    ((uint32_t)0x80000000U) /* Own Bit */

#define ETH_TX_VLAN_DISABLE ((uint32_t)(ETH_DMATXND2RF_VTIR_0 \
                                      & ETH_DMATXND2RF_VTIR_1)) /* Do not add a VLAN tag */
#define ETH_TX_VLAN_REMOVE  ((uint32_t)(ETH_DMATXND2RF_VTIR_0)) /* Remove the VLAN tag from the packets before transmission */
#define ETH_TX_VLAN_INSERT  ((uint32_t)(ETH_DMATXND2RF_VTIR_1)) /* Insert a VLAN tag */
#define ETH_TX_VLAN_REPLACE ((uint32_t)(ETH_DMATXND2RF_VTIR_0 \
                                      | ETH_DMATXND2RF_VTIR_1)) /* Replace the VLAN tag */

#define ETH_CHECKSUM_INSERT_DISABIE                 ((uint32_t)(ETH_DMATXND3RF_CIC_0 \
                                                              & ETH_DMATXND3RF_CIC_1)) /* Do Nothing: Checksum Engine is disabled */
#define ETH_CHECKSUM_INSERT_ONLY_IPHDR              ((uint32_t)(ETH_DMATXND3RF_CIC_0)) /* Only IP header checksum calculation and insertion are enabled */
#define ETH_CHECKSUM_INSERT_IPHDR_PAYLOAD           ((uint32_t)(ETH_DMATXND3RF_CIC_1)) /* IP header checksum and payload checksum calculation and insertion are
...
#define ETH_CHECKSUM_INSERT_IPHDR_PAYLOAD_PHDR_CALC ((uint32_t)(ETH_DMATXND3RF_CIC_0 \
                                                              | ETH_DMATXND3RF_CIC_1)) /* IP Header checksum and payload checksum calculation and insertion are
                                                                                          enabled, and pseudo header checksum is calculated in hardware */

#define ETH_SRC_ADDR_CONTROL_DISABLE ((uint32_t)(ETH_DMATXND3RF_SAIC_0 \
                                               & ETH_DMATXND3RF_SAIC_1)) /* Do not include the source address */
#define ETH_SRC_ADDR_INSERT          ((uint32_t)(ETH_DMATXND3RF_SAIC_0)) /* Include or insert the source address */
#define ETH_SRC_ADDR_REPLACE         ((uint32_t)(ETH_DMATXND3RF_SAIC_1)) /* Replace the source address */

#define ETH_CRC_PAD_INSERT     ((uint32_t)(ETH_DMATXND3RF_CPC_0 \
                                         & ETH_DMATXND3RF_CPC_1)) /* CRC and Pad Insertion */
#define ETH_CRC_INSERT         ((uint32_t)(ETH_DMATXND3RF_CPC_0)) /* CRC Insertion (Disable Pad Insertion) */
#define ETH_CRC_INSERT_DISABIE ((uint32_t)(ETH_DMATXND3RF_CPC_1)) /* Disable CRC Insertion */
#define ETH_CRC_REPLACEMENT    ((uint32_t)(ETH_DMATXND3RF_CPC_0 \
                                         | ETH_DMATXND3RF_CPC_1)) /* CRC Replacement */
...
#define ETH_DMATXND0WBF_TTSL ((uint32_t)0xFFFFFFFFU) /* Transmit Packet Timestamp Low */

/** Bit definition for ETH_DMATXND1WBF register (TDES1) **/
#define ETH_DMATXND1WBF_TTSH ((uint32_t)0xFFFFFFFFU) /* Transmit Packet Timestamp High */

/** Bit definition for ETH_DMATXND3WBF register (TDES3) **/
#define ETH_DMATXND3WBF_IHE  ((uint32_t)0x00000001U) /* IP Header Error */
#define ETH_DMATXND3WBF_DB   ((uint32_t)0x00000002U) /* Deferred Bit */
#define ETH_DMATXND3WBF_UF   ((uint32_t)0x00000004U) /* Underflow Error */
#define ETH_DMATXND3WBF_ED   ((uint32_t)0x00000004U) /* Excessive Deferral */
#define ETH_DMATXND3WBF_CC   ((uint32_t)0x000000F0U) /* Collision Count */
#define ETH_DMATXND3WBF_EC   ((uint32_t)0x00000100U) /* Excessive Collision */
#define ETH_DMATXND3WBF_LC   ((uint32_t)0x00000200U) /* Late Collision */
#define ETH_DMATXND3WBF_NC   ((uint32_t)0x00000400U) /* No Carrier */
#define ETH_DMATXND3WBF_LOC  ((uint32_t)0x00000800U) /* Loss of Carrier */
#define ETH_DMATXND3WBF_PCE  ((uint32_t)0x00001000U) /* Payload Checksum Error */
#define ETH_DMATXND3WBF_PF   ((uint32_t)0x00002000U) /* Packet Flushed */
#define ETH_DMATXND3WBF_JT   ((uint32_t)0x00004000U) /* Jabber Timeout */
#define ETH_DMATXND3WBF_ES   ((uint32_t)0x00008000U) /* Error Summary */
#define ETH_DMATXND3WBF_EUE  ((uint32_t)0x00010000U) /* ECC Uncorrectable Error Status in TSO memory */
#define ETH_DMATXND3WBF_TTSS ((uint32_t)0x00020000U) /* Tx Timestamp Status */
#define ETH_DMATXND3WBF_DE   ((uint32_t)0x00800000U) /* Descriptor Error */
#define ETH_DMATXND3WBF_LD   ((uint32_t)0x10000000U) /* Last Descriptor */
#define ETH_DMATXND3WBF_FD   ((uint32_t)0x20000000U) /* First Descriptor */
#define ETH_DMATXND3WBF_CTXT ((uint32_t)0x40000000U) /* Context Type */
#define ETH_DMATXND3WBF_OWN  ((uint32_t)0x80000000U) /* Own Bit */
...
#define ETH_DMATXCD0_TTSL ((uint32_t)0xFFFFFFFFU) /* Transmit Packet Timestamp Low */

/** Bit definition for ETH_DMATXCD1 register (TDES1) **/
#define ETH_DMATXCD1_TTSH ((uint32_t)0xFFFFFFFFU) /* Transmit Packet Timestamp High */

/** Bit definition for ETH_DMATXCD2 register (TDES2) **/
#define ETH_DMATXCD2_MSS  ((uint32_t)0x00003FFFU) /* Maximum Segment Size */
#define ETH_DMATXCD2_IVT  ((uint32_t)0xFFFF0000U) /* Inner VLAN Tag */

/** Bit definition for ETH_DMATXCD3 register (TDES3) **/
#define ETH_DMATXCD3_VT      ((uint32_t)0x0000FFFFU) /* VLAN Tag */
#define ETH_DMATXCD3_VLTV    ((uint32_t)0x00010000U) /* VLAN Tag Valid */
#define ETH_DMATXCD3_IVLTV   ((uint32_t)0x00020000U) /* Inner VLAN Tag Valid */
#define ETH_DMATXCD3_IVTIR   ((uint32_t)0x000C0000U) /* IVTIR[1:0]: Inner VLAN Tag Insert or Replace */
#define ETH_DMATXCD3_IVTIR_0 ((uint32_t)0x00040000U) /* IVTIR bit 0 */
#define ETH_DMATXCD3_IVTIR_1 ((uint32_t)0x00080000U) /* IVTIR bit 1 */
#define ETH_DMATXCD3_CDE     ((uint32_t)0x00800000U) /* Context Descriptor Error */
#define ETH_DMATXCD3_TCMSSV  ((uint32_t)0x04000000U) /* One-Step Timestamp Correction Input or MSS Valid */
#define ETH_DMATXCD3_OSTC    ((uint32_t)0x08000000U) /* One-Step Timestamp Correction Enable */
#define ETH_DMATXCD3_CTXT    ((uint32_t)0x40000000U) /* Context Type */
#define ETH_DMATXCD3_OWN     ((uint32_t)0x80000000U) /* Own Bit */

#define ETH_TX_INNER_VLAN_DISABLE ((uint32_t)(ETH_DMATXCD3_IVTIR_0 \
                                            & ETH_DMATXCD3_IVTIR_1)) /* Do not add a inner VLAN tag */
#define ETH_TX_INNER_VLAN_REMOVE  ((uint32_t)(ETH_DMATXCD3_IVTIR_0)) /* Remove the inner VLAN tag from the packets before transmission */
#define ETH_TX_INNER_VLAN_INSERT  ((uint32_t)(ETH_DMATXCD3_IVTIR_1)) /* Insert a inner VLAN tag */
#define ETH_TX_INNER_VLAN_REPLACE ((uint32_t)(ETH_DMATXCD3_IVTIR_0 \
                                            | ETH_DMATXCD3_IVTIR_1)) /* Replace the inner VLAN tag */
...
#define ETH_DMARXND0RF_BUF1AP ((uint32_t)0xFFFFFFFFU) /* Buffer 1 Address Pointer */

/** Bit definition for ETH_DMARXND2RF register (RDES2) **/
#define ETH_DMARXND2RF_BUF2AP ((uint32_t)0xFFFFFFFFU) /* Buffer 2 Address Pointer */

/** Bit definition for ETH_DMARXND3RF register (RDES3) **/
#define ETH_DMARXND3RF_BUF1V ((uint32_t)0x01000000U) /* Buffer 1 Address Valid */
#define ETH_DMARXND3RF_BUF2V ((uint32_t)0x02000000U) /* Buffer 2 Address Valid */
#define ETH_DMARXND3RF_IOC   ((uint32_t)0x40000000U) /* Interrupt Enabled on Completion */
#define ETH_DMARXND3RF_OWN   ((uint32_t)0x80000000U) /* Own Bit */
...
#define ETH_DMARXND0WBF_OVT ((uint32_t)0x0000FFFFU) /* Outer VLAN Tag */
#define ETH_DMARXND0WBF_IVT ((uint32_t)0xFFFF0000U) /* Inner VLAN Tag */

/** Bit definition for ETH_DMARXND1WBF register (RDES1) **/
#define ETH_DMARXND1WBF_PT    ((uint32_t)0x00000007U) /* PT[2:0]: Payload Type */
#define ETH_DMARXND1WBF_PT_0  ((uint32_t)0x00000001U) /* PT bit 0 */
#define ETH_DMARXND1WBF_PT_1  ((uint32_t)0x00000002U) /* PT bit 1 */
#define ETH_DMARXND1WBF_PT_2  ((uint32_t)0x00000004U) /* PT bit 2 */
#define ETH_DMARXND1WBF_IPHE  ((uint32_t)0x00000008U) /* IP Header Error */
#define ETH_DMARXND1WBF_IPV4  ((uint32_t)0x00000010U) /* IPV4 Header Present */
#define ETH_DMARXND1WBF_IPV6  ((uint32_t)0x00000020U) /* IPV6 Header Present */
#define ETH_DMARXND1WBF_IPCB  ((uint32_t)0x00000040U) /* IP Checksum Bypassed */
#define ETH_DMARXND1WBF_IPCE  ((uint32_t)0x00000080U) /* IP Payload Error */
#define ETH_DMARXND1WBF_PMT   ((uint32_t)0x00000F00U) /* PMT[3:0]: PTP Message Type */
#define ETH_DMARXND1WBF_PMT_0 ((uint32_t)0x00000100U) /* PMT bit 0 */
#define ETH_DMARXND1WBF_PMT_1 ((uint32_t)0x00000200U) /* PMT bit 1 */
#define ETH_DMARXND1WBF_PMT_2 ((uint32_t)0x00000400U) /* PMT bit 2 */
#define ETH_DMARXND1WBF_PMT_3 ((uint32_t)0x00000800U) /* PMT bit 3 */
#define ETH_DMARXND1WBF_PFT   ((uint32_t)0x00001000U) /* PTP Packet Type */
#define ETH_DMARXND1WBF_PV    ((uint32_t)0x00002000U) /* PTP Version */
#define ETH_DMARXND1WBF_TSA   ((uint32_t)0x00004000U) /* Timestamp Available */
#define ETH_DMARXND1WBF_TD    ((uint32_t)0x00008000U) /* Timestamp Dropped */
#define ETH_DMARXND1WBF_OPC   ((uint32_t)0xFFFF0000U) /* OAM Sub-Type Code, or MAC Control Packet opcode */
...
#define ETH_DMARXDESC_EXTFLAG_PT_UDP     ((uint32_t)(ETH_DMARXND1WBF_PT_0)) /* Payload Type: UDP */
#define ETH_DMARXDESC_EXTFLAG_PT_TCP     ((uint32_t)(ETH_DMARXND1WBF_PT_1)) /* Payload Type: TCP */
#define ETH_DMARXDESC_EXTFLAG_PT_ICMP    ((uint32_t)(ETH_DMARXND1WBF_PT_0 \
...
#define ETH_IP_PAYLOAD_TYPE_UNKNOWN ((uint32_t)(ETH_DMARXDESC_EXTFLAG_PT_NONE))
#define ETH_IP_PAYLOAD_TYPE_UDP     ((uint32_t)(ETH_DMARXDESC_EXTFLAG_PT_UDP))
#define ETH_IP_PAYLOAD_TYPE_TCP     ((uint32_t)(ETH_DMARXDESC_EXTFLAG_PT_TCP))
#define ETH_IP_PAYLOAD_TYPE_ICMPN   ((uint32_t)(ETH_DMARXDESC_EXTFLAG_PT_ICMP))

#define ETH_IP_HEADER_IPV4 ((uint32_t)(ETH_DMARXDESC_EXTFLAG_IPV4))
#define ETH_IP_HEADER_IPV6 ((uint32_t)(ETH_DMARXDESC_EXTFLAG_IPV6))

#define ETH_CHECKSUM_BYPASSED         ((uint32_t)(ETH_DMARXND1WBF_IPCB))
#define ETH_CHECKSUM_IP_HEADER_ERROR  ((uint32_t)(ETH_DMARXND1WBF_IPHE))
#define ETH_CHECKSUM_IP_PAYLOAD_ERROR ((uint32_t)(ETH_DMARXND1WBF_IPCE))


/** Bit definition for ETH_DMARXND2WBF register (RDES2) **/
#define ETH_DMARXND2WBF_ARPNR    ((uint32_t)0x00000400U) /* ARP Reply Not Generated */
#define ETH_DMARXND2WBF_VFS      ((uint32_t)0x00008000U) /* VLAN Filter Status */
#define ETH_DMARXND2WBF_SAF      ((uint32_t)0x00010000U) /* SA Address Filter Fail */
#define ETH_DMARXND2WBF_DAF      ((uint32_t)0x00020000U) /* DA Address Filter Fail */
#define ETH_DMARXND2WBF_HF       ((uint32_t)0x00040000U) /* Hash Filter Status */
#define ETH_DMARXND2WBF_MADRM    ((uint32_t)0x07F80000U) /* MAC Address Match */
#define ETH_DMARXND2WBF_HV       ((uint32_t)0x07F80000U) /* Hash Value */
#define ETH_DMARXND2WBF_L3FM     ((uint32_t)0x08000000U) /* Layer 3 Filter Match */
#define ETH_DMARXND2WBF_L4FM     ((uint32_t)0x10000000U) /* Layer 4 Filter Match */
#define ETH_DMARXND2WBF_L3L4FM   ((uint32_t)0xE0000000U) /* L3L4FM[2:0]: Layer 3 and Layer 4 Filter Number Matched */
#define ETH_DMARXND2WBF_L3L4FM_0 ((uint32_t)0x20000000U) /* L3L4FM bit 0 */
#define ETH_DMARXND2WBF_L3L4FM_1 ((uint32_t)0x40000000U) /* L3L4FM bit 1 */
#define ETH_DMARXND2WBF_L3L4FM_2 ((uint32_t)0x80000000U) /* L3L4FM bit 2 */
...
#define ETH_HASH_FILTER_PASS    ((uint32_t)(ETH_DMARXND2WBF_HF))
#define ETH_VLAN_FILTER_PASS    ((uint32_t)(ETH_DMARXND2WBF_VFS))
#define ETH_DEST_ADDRESS_FAIL   ((uint32_t)(ETH_DMARXND2WBF_DAF))
#define ETH_SOURCE_ADDRESS_FAIL ((uint32_t)(ETH_DMARXND2WBF_SAF))

#define ETH_L3_FILTER0_MATCH ((uint32_t)(ETH_DMARXND2WBF_L3FM))
#define ETH_L3_FILTER1_MATCH ((uint32_t)(ETH_DMARXND2WBF_L3FM \
                                       | ETH_DMARXND2WBF_L3L4FM_0))
#define ETH_L3_FILTER2_MATCH ((uint32_t)(ETH_DMARXND2WBF_L3FM \
                                       | ETH_DMARXND2WBF_L3L4FM_1))
#define ETH_L3_FILTER3_MATCH ((uint32_t)(ETH_DMARXND2WBF_L3FM \
                                       | ETH_DMARXND2WBF_L3L4FM_0 \
                                       | ETH_DMARXND2WBF_L3L4FM_1))
#define ETH_L3_FILTER4_MATCH ((uint32_t)(ETH_DMARXND2WBF_L3FM \
                                       | ETH_DMARXND2WBF_L3L4FM_2))
#define ETH_L3_FILTER5_MATCH ((uint32_t)(ETH_DMARXND2WBF_L3FM \
                                       | ETH_DMARXND2WBF_L3L4FM_0 \
                                       | ETH_DMARXND2WBF_L3L4FM_2))
#define ETH_L3_FILTER6_MATCH ((uint32_t)(ETH_DMARXND2WBF_L3FM \
                                       | ETH_DMARXND2WBF_L3L4FM_1 \
                                       | ETH_DMARXND2WBF_L3L4FM_2))
#define ETH_L3_FILTER7_MATCH ((uint32_t)(ETH_DMARXND2WBF_L3FM \
                                       | ETH_DMARXND2WBF_L3L4FM_0 \
                                       | ETH_DMARXND2WBF_L3L4FM_1 \
                                       | ETH_DMARXND2WBF_L3L4FM_2))

#define ETH_L4_FILTER0_MATCH ((uint32_t)(ETH_DMARXND2WBF_L4FM))
#define ETH_L4_FILTER1_MATCH ((uint32_t)(ETH_DMARXND2WBF_L4FM \
                                       | ETH_DMARXND2WBF_L3L4FM_0))
#define ETH_L4_FILTER2_MATCH ((uint32_t)(ETH_DMARXND2WBF_L4FM \
                                       | ETH_DMARXND2WBF_L3L4FM_1))
#define ETH_L4_FILTER3_MATCH ((uint32_t)(ETH_DMARXND2WBF_L4FM \
                                       | ETH_DMARXND2WBF_L3L4FM_0 \
                                       | ETH_DMARXND2WBF_L3L4FM_1))
#define ETH_L4_FILTER4_MATCH ((uint32_t)(ETH_DMARXND2WBF_L4FM \
                                       | ETH_DMARXND2WBF_L3L4FM_2))
#define ETH_L4_FILTER5_MATCH ((uint32_t)(ETH_DMARXND2WBF_L4FM \
                                       | ETH_DMARXND2WBF_L3L4FM_0 \
                                       | ETH_DMARXND2WBF_L3L4FM_2))
#define ETH_L4_FILTER6_MATCH ((uint32_t)(ETH_DMARXND2WBF_L4FM \
                                       | ETH_DMARXND2WBF_L3L4FM_1 \
                                       | ETH_DMARXND2WBF_L3L4FM_2))
#define ETH_L4_FILTER7_MATCH ((uint32_t)(ETH_DMARXND2WBF_L4FM \
                                       | ETH_DMARXND2WBF_L3L4FM_0 \
                                       | ETH_DMARXND2WBF_L3L4FM_1 \
                                       | ETH_DMARXND2WBF_L3L4FM_2))
...
#define ETH_DMARXND3WBF_PL   ((uint32_t)0x00007FFFU) /* Packet Length */
#define ETH_DMARXND3WBF_ES   ((uint32_t)0x00008000U) /* Error Summary */
#define ETH_DMARXND3WBF_LT   ((uint32_t)0x00070000U) /* LT[2:0]: Length/Type Field */
#define ETH_DMARXND3WBF_LT_0 ((uint32_t)0x00010000U) /* LT bit 0 */
#define ETH_DMARXND3WBF_LT_1 ((uint32_t)0x00020000U) /* LT bit 1 */
#define ETH_DMARXND3WBF_LT_2 ((uint32_t)0x00040000U) /* LT bit 2 */
#define ETH_DMARXND3WBF_DE   ((uint32_t)0x00080000U) /* Dribble Bit Error */
#define ETH_DMARXND3WBF_RE   ((uint32_t)0x00100000U) /* Receive Error */
#define ETH_DMARXND3WBF_OE   ((uint32_t)0x00200000U) /* Overflow Error */
#define ETH_DMARXND3WBF_RWT  ((uint32_t)0x00400000U) /* Receive Watchdog Timeout */
#define ETH_DMARXND3WBF_GP   ((uint32_t)0x00800000U) /* Giant Packet */
#define ETH_DMARXND3WBF_CE   ((uint32_t)0x01000000U) /* CRC Error */
#define ETH_DMARXND3WBF_RS0V ((uint32_t)0x02000000U) /* Receive Status RDES0 Valid */
#define ETH_DMARXND3WBF_RS1V ((uint32_t)0x04000000U) /* Receive Status RDES1 Valid */
#define ETH_DMARXND3WBF_RS2V ((uint32_t)0x08000000U) /* Receive Status RDES2 Valid */
#define ETH_DMARXND3WBF_LD   ((uint32_t)0x10000000U) /* Last Descriptor */
#define ETH_DMARXND3WBF_FD   ((uint32_t)0x20000000U) /* First Descriptor */
#define ETH_DMARXND3WBF_CTXT ((uint32_t)0x40000000U) /* Receive Context Descriptor */
#define ETH_DMARXND3WBF_OWN  ((uint32_t)0x80000000U) /* Own Bit */
...
#define ETH_DMARXCD0_RTSL ((uint32_t)0xFFFFFFFFU) /* Receive Packet Timestamp Low */

/** Bit definition for ETH_DMARXCD1 register (RDES1) **/
#define ETH_DMARXCD1_RTSH ((uint32_t)0xFFFFFFFFU) /* Receive Packet Timestamp High */

/** Bit definition for ETH_DMARXCD3 register (RDES3) **/
#define ETH_DMARXCD3_DE   ((uint32_t)0x20000000U) /* Descriptor Error */
#define ETH_DMARXCD3_CTXT ((uint32_t)0x40000000U) /* Receive Context Descriptor */
#define ETH_DMARXCD3_OWN  ((uint32_t)0x80000000U) /* Own Bit */
...
void ETH_RxVLANTagStructInit(ETH_RxVLANTagType* ETH_RxVTInitParam);
EthFuncStatusType ETH_ConfigRxVLANTag(ETH_Module* ETHx, ETH_RxVLANTagType* pRxVLANTag);
void ETH_SetVLANHashTable(ETH_Module* ETHx, uint16_t HashTable);
void ETH_TxVLANTagStructInit(ETH_TxVLANTagType* ETH_TxVTInitParam);
EthFuncStatusType ETH_ConfigTxOuterVLANTag(ETH_Module* ETHx, ETH_TxVLANTagType* pTxVLANTag);
EthFuncStatusType ETH_ConfigTxInnerVLANTag(ETH_Module* ETHx, ETH_TxVLANTagType* pTxVLANTag);
void ETH_SetRxVLANIdentifier(ETH_Module* ETHx, uint32_t VLANIdentifier);
void ETH_SetTxOuterVLANIdentifier(ETH_Module* ETHx, uint32_t VLANIdentifier);
void ETH_SetTxInnerVLANIdentifier(ETH_Module* ETHx, uint32_t VLANIdentifier);
void ETH_VLANFilteringCmd(ETH_Module* ETHx, FunctionalStatus Cmd);
...
void ETH_EnterLPIMode(ETH_Module* ETHx, EthLPITxClkType TxClockStop, EthLPITxAutoType TxAutomate);
void ETH_ExitLPIMode(ETH_Module* ETHx);

void ETH_SetARPAddressMatch(ETH_Module* ETHx, uint32_t IpAddress);
...
void ETH_MACAddress1Filter(ETH_Module* ETHx, EthMacAddrFilterType Filter, uint32_t MaskByte, uint8_t* pAddr);
void ETH_MACAddress2Filter(ETH_Module* ETHx, EthMacAddrFilterType Filter, uint32_t MaskByte, uint8_t* pAddr);
void ETH_MACAddress3Filter(ETH_Module* ETHx, EthMacAddrFilterType Filter, uint32_t MaskByte, uint8_t* pAddr);
...
EthFuncStatusType ETH_ConfigL3Filter0(ETH_Module* ETHx, ETH_L3FilterType* pL3Filter);
EthFuncStatusType ETH_ConfigL3Filter1(ETH_Module* ETHx, ETH_L3FilterType* pL3Filter);
EthFuncStatusType ETH_ConfigL4Filter0(ETH_Module* ETHx, ETH_L4FilterType* pL4Filter);
EthFuncStatusType ETH_ConfigL4Filter1(ETH_Module* ETHx, ETH_L4FilterType* pL4Filter);
void ETH_L3L4FilteringCmd(ETH_Module* ETHx, FunctionalStatus Cmd);
...
uint16_t ETH_GetRxDataLength(ETH_InfoType* pInfo);
